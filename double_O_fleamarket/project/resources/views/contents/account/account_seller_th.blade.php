		...
		FusionCharts.ready(function(){
		    var fusioncharts = new FusionCharts({
			    ...
			    dataSource: {
			        ...
			        "categories": [{
			            "category": [
			            	@for($i = 0, $length = count($accountInfo['goodsInfo']); $i < $length; $i++)
			            	{
				            	"label":"{{$accountInfo['goodsInfo'][$i]->name}}"
				            },
				            @endfor
				        ]
			        }],
			        "dataset": [{
			            ...
			            "data": [
				            @for($i = 0, $length = count($accountInfo['goodsInfo']); $i < $length; $i++)
				            {
				         	   'value':"{{$accountInfo['goodsInfo'][$i]->account}}"
				        	},
				            @endfor
			            ]
			        }]
...
<body>
	...
	<!--정산 wrapper-->
		...
		<div class="account_accountArea account_divBox">
			<div style="margin-left: 10px; margin-top: 30px; height: 60px;">
				<select class="account_select account_divBox" style="width: 180px; height: 31px;">
					<option value="per_fleamarket_account">물품별 총 수익</option>
				</select>
		...
		<div class="account_detailArea account_divBox">
			<div class="text-center detailArea_header account_BottomBox">{{$accountInfo['fleamarketInfo'][0]->flea_name}} {{$accountInfo['th']}}차</div>
			<div class="detailArea_body">
				<div class="account_BottomBox detailArea_content">
					<li class="text-left detailArea_content_title">물품별 최고 실적</li>
					<li class="text-right detailArea_content_text">{{number_format($accountInfo['maxAcc'])}} 원</li>
				</div>
				<div class="account_BottomBox detailArea_content">
					<li class="text-left detailArea_content_title">물품별 최저 실적</li>
				...
				<div class="account_BottomBox detailArea_content">
					...
					<li class="text-right detailArea_content_text">{{number_format($accountInfo['booth_fee'])}} 원</li>
				</div>
				<div class="account_BottomBox detailArea_content">
